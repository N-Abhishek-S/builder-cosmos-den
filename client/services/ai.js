...
export async function analyzeImage(file) {
  ...
  const recSunglasses = products
    .filter((p) => p.type === "sunglasses" && wantedStyles.includes(p.style))
    .slice(0, 12);

  const recOutfits = products
    .filter((p) => p.type === "outfit")
    .filter((p) => p.palette.some((hex) => palettes[skin].includes(hex)))
    .slice(0, 8);
